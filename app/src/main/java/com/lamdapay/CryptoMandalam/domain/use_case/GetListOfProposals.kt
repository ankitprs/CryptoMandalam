...
class GetListOfProposals @Inject constructor(
    private val repository: FirestoreRepo
)  {
    operator fun invoke(userWalletAddress: String): Flow<Resource<List<Proposal>>> = flow {
        try {
            emit(Resource.Loading())
            val proposalList = repository.getListOfProposals(userWalletAddress)
